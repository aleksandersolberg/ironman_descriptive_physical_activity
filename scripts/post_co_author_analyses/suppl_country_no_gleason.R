...
# Purpose: Baseline descriptives by country region
#--------------------------------------------------------------------------------
# Load packages
...
table_1 <- baseline_unique %>%
  tbl_summary(
    include = c(
      age, BMI_subgroup, country_region,
...
table_1 %>%
  ...
  save_as_docx(path = here("output", "supplemental", "baseline_descriptives", "suppl_table_1_by_country.docx"))
